...
void stmt_fprint(FILE* fp, struct stmt* s, int indent) {
   ...
   */
    case STMT_IF_ELSE:
      fprintf(fp, "if ("); expr_fprint(fp, s->expr); fprintf(fp, ") ");
      ...
      if (s->else_body) {
        if (s->else_body->kind == STMT_BLOCK) { fprintf(fp, " else "); stmt_fprint(fp, s->else_body, indent); }
        else {
   ...
   */
    ...
    case STMT_PRINT: 
      fprintf(fp, "print"); if (s->expr) { fprintf(fp, " "); }
      expr_fprint(fp, s->expr); fprintf(fp, ";");
		break;

    // return; return expr; return expr, expr;
    case STMT_RETURN:
      fprintf(fp, "return"); if (s->expr) { fprintf(fp, " "); }
      expr_fprint(fp, s->expr); fprintf(fp, ";");
		break;
    /*
    {
    ...
    */
    case STMT_WHILE:
      fprintf(fp, "while ("); expr_fprint(fp, s->expr); fprintf(fp, ") ");
...
int stmt_resolve(struct symbol_table* st, struct stmt* s) {
  ...
  switch (s->kind) {
    ...
    case STMT_FOR:
      ...
      // FIXME: if there is declaration it is to be part of the body's scope!!!
...
int stmt_typecheck(struct symbol_table* st, struct stmt* s, struct type** ret_type) {
  ...
  switch(s->kind) {
    ...
    case STMT_IF_ELSE:
      ...
      if (s->expr->kind == EXPR_COMMA || s->expr->kind == EXPR_ASSIGN) {
        error_status = stmt_error_handle(STMT_BOOLEAN, (void*)s, (void*)t);
      }
      type_destroy(&t);
      if (s->body && s->body->kind != STMT_BLOCK) { symbol_table_scope_enter(st); }
      error_status = stmt_typecheck(st, s->body, ret_type);
